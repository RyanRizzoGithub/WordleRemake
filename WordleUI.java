...
public class WordleUI {
	...
	public WordleUI(WordleGame currGame) {
		...
		endGameShell = new Shell(display);
		endGameShell.setText("Stats");
		endGameShell.setSize(400, 400);
		endGameShell.setLayout(new GridLayout());
	...
	// Katelen Tellez
	public void endGame(PaintEvent e) {
		
		System.out.println("In end game...");
		
		Composite endGameComp = new Composite(endGameShell, SWT.NO_FOCUS);
		canvas = new Canvas(endGameComp, SWT.NONE);
		canvas.setSize(400, 400);
		
		// Event loop
		endGameShell.open();
		while (!endGameShell.isDisposed())
			if (!display.readAndDispatch()) {
			}
		display.dispose();
	}
